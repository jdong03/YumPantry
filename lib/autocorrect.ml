...
let rec match_string_helper (s : string) (lst : Ingredient.t list)
    (acc : int * Ingredient.t) : int * Ingredient.t =
  match lst with
  | [] -> acc
  | h :: t -> (
...
let match_string (s : string) : Ingredient.t option =
  let first_ingredient = List.hd Ingredient.all_ingredients in
  match
    match_string_helper s Ingredient.all_ingredients
      (String.length s, first_ingredient)
  with
